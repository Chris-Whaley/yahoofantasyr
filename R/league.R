...
yf_league_standings <- function(league_key) {
  ...
  for (team in names(teams)) {
    if (team != "count") {
      ...
      #   # tibble::as_tibble() |>
      #   clean_yahoo_list() |>
      #   dplyr::mutate(dplyr::across(!c("team_key", "name", "nickname"), ~ purrr::map_dbl(.x, as.numeric)))
      temp_df_meta <- yf_team(find_variable_in_nested_list(team_df[[1]], "team_key"))
      temp_df_scores <- purrr::map(vars_scores, ~ yahoofantasyr::find_variable_in_nested_list(team_df[[3]], .x)) |>
        rlang::set_names(vars_scores) |>
        # tibble::as_tibble() |>
        clean_yahoo_list() |>
        dplyr::mutate(dplyr::across(!c("type"), ~ purrr::map_dbl(.x, as.numeric)))
...
## TODO: test NHL. nfl and nba work
#' Get the Scoreboard for All Teams
#'
#' @param league_key Identifier for league.
#' @param week Week number of league
#'
#' @returns Dataframe
#' @export
#'
#' @examples yf_league_scoreboard("466.l.278123", week = 1)
yf_league_scoreboard <- function(league_key, week = NULL) {
  ...
  df <- df |>
    dplyr::mutate(
      week = ifelse(is.null(week), metadata$week, week),
      week_start = metadata$week_start,
      week_end = metadata$week_end
    ) |>
    dplyr::relocate(week, week_start, week_end)
...
yf_league_teams <- function(league_key) {
  ...
  for (team in names(data)) {
    if (team == "count") {} else {
      temp_df <- purrr::map(vars, ~ find_variable_in_nested_list(data[[team]], .x)) %>%
...
extract_matchup_teams <- function(matchup) {
  ...
  home_team <- clean_yahoo_list(home_team)
  away_team <- clean_yahoo_list(away_team)

  df <- dplyr::bind_rows(home_team, away_team)
